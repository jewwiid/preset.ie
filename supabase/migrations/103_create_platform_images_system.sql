...
-- Create platform-images storage bucket
...
-- Create RLS policies for platform-images bucket
CREATE POLICY "Public can view platform images" ON storage.objects
FOR SELECT USING (bucket_id = 'platform-images');

CREATE POLICY "Service role can upload platform images" ON storage.objects
FOR INSERT WITH CHECK (
  bucket_id = 'platform-images' 
  AND auth.role() = 'service_role'
);

CREATE POLICY "Service role can update platform images" ON storage.objects
FOR UPDATE USING (
  bucket_id = 'platform-images' 
  AND auth.role() = 'service_role'
);

CREATE POLICY "Service role can delete platform images" ON storage.objects
FOR DELETE USING (
  bucket_id = 'platform-images' 
  AND auth.role() = 'service_role'
);
