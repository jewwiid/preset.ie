-- SAFE VERSION: Rename user_id to profile_id in detail tables for clarity
...
-- Step 1: Verify data integrity before making changes
DO $$
DECLARE
    invalid_count INTEGER;
BEGIN
    -- Check for any user_equipment records with user_id that don't match a profile
    ...
    IF invalid_count > 0 THEN
        RAISE EXCEPTION 'Found % user_equipment records with invalid user_id references', invalid_count;
    END IF;
    
    -- Check for any user_clothing_sizes records with user_id that don't match a profile
    SELECT COUNT(*) INTO invalid_count
    FROM user_clothing_sizes ucs
    LEFT JOIN users_profile up ON ucs.user_id = up.id
    WHERE up.id IS NULL;
    
    IF invalid_count > 0 THEN
        RAISE EXCEPTION 'Found % user_clothing_sizes records with invalid user_id references', invalid_count;
    END IF;
    
    -- Check for any user_measurements records with user_id that don't match a profile
    SELECT COUNT(*) INTO invalid_count
    FROM user_measurements um
    LEFT JOIN users_profile up ON um.user_id = up.id
    WHERE up.id IS NULL;
    
    IF invalid_count > 0 THEN
        RAISE EXCEPTION 'Found % user_measurements records with invalid user_id references', invalid_count;
    END IF;
    
    -- Check for any user_shoe_sizes records with user_id that don't match a profile
    SELECT COUNT(*) INTO invalid_count
    FROM user_shoe_sizes uss
    LEFT JOIN users_profile up ON uss.user_id = up.id
    WHERE up.id IS NULL;
    
    IF invalid_count > 0 THEN
        RAISE EXCEPTION 'Found % user_shoe_sizes records with invalid user_id references', invalid_count;
    END IF;
    
    RAISE NOTICE 'Data integrity check passed - all user_id references are valid';
END $$;

-- Step 2: Rename user_id to profile_id in user_equipment table
ALTER TABLE user_equipment 
RENAME COLUMN user_id TO profile_id;

-- Update the foreign key constraint name
ALTER TABLE user_equipment 
DROP CONSTRAINT IF EXISTS user_equipment_user_id_fkey;
...
-- Step 3: Rename user_id to profile_id in user_clothing_sizes table
ALTER TABLE user_clothing_sizes 
RENAME COLUMN user_id TO profile_id;

-- Update the foreign key constraint name
ALTER TABLE user_clothing_sizes 
DROP CONSTRAINT IF EXISTS user_clothing_sizes_user_id_fkey;
...
-- Step 4: Rename user_id to profile_id in user_measurements table
ALTER TABLE user_measurements 
RENAME COLUMN user_id TO profile_id;

-- Update the foreign key constraint name
ALTER TABLE user_measurements 
DROP CONSTRAINT IF EXISTS user_measurements_user_id_fkey;
...
-- Step 5: Rename user_id to profile_id in user_shoe_sizes table
ALTER TABLE user_shoe_sizes 
RENAME COLUMN user_id TO profile_id;

-- Update the foreign key constraint name
ALTER TABLE user_shoe_sizes 
DROP CONSTRAINT IF EXISTS user_shoe_sizes_user_id_fkey;
