...
SELECT 
    ...
    CASE 
        WHEN sp.style_type = 'photorealistic' THEN 'photography'
        WHEN sp.style_type = 'artistic' THEN 'artistic'
        WHEN sp.style_type = 'cartoon' THEN 'artistic'
        WHEN sp.style_type = 'vintage' THEN 'artistic'
        WHEN sp.style_type = 'cyberpunk' THEN 'artistic'
        WHEN sp.style_type = 'watercolor' THEN 'artistic'
        WHEN sp.style_type = 'sketch' THEN 'artistic'
        WHEN sp.style_type = 'oil_painting' THEN 'artistic'
        ELSE 'custom'
    END as category,
...
WHERE NOT EXISTS (
    ...
    AND p.creator_id = sp.user_id
);

-- Update the migrated presets to have proper creator information
UPDATE presets 
SET creator = jsonb_build_object(
    'id', up.user_id,
    'display_name', COALESCE(up.display_name, up.handle, 'Unknown User'),
    'handle', up.handle,
    'avatar_url', up.avatar_url
)
FROM users_profile up
WHERE presets.creator_id = up.user_id
AND presets.ai_metadata->>'migrated_from_style_preset' = 'true';

-- Add a comment to track the migration
COMMENT ON TABLE presets IS 'Comprehensive presets table - migrated from playground_style_presets in migration 100';

-- Create an index for better performance on migrated presets
CREATE INDEX IF NOT EXISTS idx_presets_migrated_from_style 
ON presets USING GIN ((ai_metadata->>'migrated_from_style_preset'));
