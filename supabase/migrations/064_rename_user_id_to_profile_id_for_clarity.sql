-- Rename user_id to profile_id in detail tables for clarity
...
-- 1. Rename user_id to profile_id in user_equipment table
ALTER TABLE user_equipment 
RENAME COLUMN user_id TO profile_id;

-- Update the foreign key constraint name and comment
ALTER TABLE user_equipment 
...
-- 2. Rename user_id to profile_id in user_clothing_sizes table
...
-- 3. Rename user_id to profile_id in user_measurements table
...
-- 4. Rename user_id to profile_id in user_shoe_sizes table
...
-- 6. Add comprehensive schema comments for clarity
...
-- 7. Update the get_user_equipment_details function to use new column name
...
-- 8. Add a helpful view to show the ID relationships clearly
...
BEGIN
    ...
    RAISE NOTICE '- user_equipment.user_id → user_equipment.profile_id';
    RAISE NOTICE '- user_clothing_sizes.user_id → user_clothing_sizes.profile_id';
    RAISE NOTICE '- user_measurements.user_id → user_measurements.profile_id';
    RAISE NOTICE '- user_shoe_sizes.user_id → user_shoe_sizes.profile_id';
