-- Create playground tables with all required columns for the API
-- This migration ensures the playground_projects table has all columns the API expects

-- Playground projects table with all required columns
CREATE TABLE IF NOT EXISTS playground_projects (
    ...
    -- Required columns that the API expects
    aspect_ratio VARCHAR(20) DEFAULT '1:1',
    resolution VARCHAR(20) DEFAULT '1024x1024',
    status VARCHAR(20) DEFAULT 'generated' CHECK (status IN ('draft', 'generated', 'saved', 'published', 'processing')),
...
DROP POLICY IF EXISTS "Users can manage own gallery" ON playground_gallery;
DROP POLICY IF EXISTS "Users can manage own image edits" ON playground_image_edits;
DROP POLICY IF EXISTS "Users can manage own video generations" ON playground_video_generations;
