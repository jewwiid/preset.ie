-- Safe Badge System Migration - Skips existing objects
-- Run this in Supabase Dashboard SQL Editor

-- Badge Types enum (safe)
DO $$ BEGIN
...
-- Junction table for user badges (safe)
...
-- RLS Policies for badges table (safe)
...
-- RLS Policies for user_badges table (safe)
...
DROP POLICY IF EXISTS "Users can view own user badges" ON user_badges;
...
CREATE INDEX IF NOT EXISTS idx_user_badges_active ON user_badges(is_active);
...
-- Function to update updated_at timestamp (safe)
...
-- Updated trigger for updated_at (safe)
...
-- Function to award a badge to a user (safe)
...
-- Function to remove a badge from a user (safe)
...
-- Function to get user badges with badge details (safe)
...
-- Insert predefined badges (safe - will skip if they exist)
...
-- Award initial badges to existing users based on current data (safe)
DO $$
DECLARE
  user_record RECORD;
  admin_user_id UUID;
BEGIN
  ...
  LOOP
    ...
    EXCEPTION
      WHEN OTHERS THEN
        -- Skip if badge already exists or other error
        NULL;
    END;
  END LOOP;
...
-- Show results
SELECT 'Badge system setup completed successfully!' as status;
