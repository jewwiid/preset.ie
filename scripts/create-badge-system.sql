-- Badge System for Preset App - Run this in Supabase Dashboard SQL Editor
...
CREATE TABLE IF NOT EXISTS user_badges (
  ...
  awarded_by UUID REFERENCES auth.users(id), -- Admin who awarded it, NULL for automatic
...
BEGIN
  ...
  LOOP
    IF user_record.subscription_tier = 'plus' THEN
      PERFORM award_badge(user_record.id, 'plus-member', NULL, 'Active Plus subscription');
    ELSIF user_record.subscription_tier = 'PRO' THEN
      PERFORM award_badge(user_record.id, 'pro-member', NULL, 'Active Pro subscription');
    END IF;
  END LOOP;
