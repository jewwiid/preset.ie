-- Comprehensive Duplicate Offers Cleanup & Prevention - FIXED VERSION
-- This script handles existing duplicates BEFORE creating the unique index
...
-- For each user-listing combination, keep only ONE offer (most recent)
...
WITH ranked_offers AS (
  SELECT 
    ...
    created_at,
    offer_amount_cents,
    -- Rank offers by priority and recency for each user-listing combination
    ROW_NUMBER() OVER (
      PARTITION BY offerer_id, listing_id 
      ORDER BY 
        CASE status 
...
-- 7. EXECUTE CLEANUP
-- This will permanently delete duplicate offers!
DELETE FROM offers WHERE id NOT IN (SELECT id FROM offers_to_keep);

-- 8. VERIFY CLEANUP WORKED
SELECT 
...
-- 9. NOW CREATE THE UNIQUE INDEX (should work after cleanup)
-- Drop existing index if it exists
DROP INDEX IF EXISTS unique_pending_offer;
DROP INDEX IF EXISTS unique_user_listing_offer;
...
-- 12. VERIFY NO DUPLICATES REMAIN
SELECT 
  'DUPLICATE CHECK:' as check_type,
  COUNT(*) as duplicate_combinations
FROM (
  SELECT offerer_id, listing_id, COUNT(*) as cnt
  FROM offers
  GROUP BY offerer_id, listing_id
  HAVING COUNT(*) > 1
) duplicates;
