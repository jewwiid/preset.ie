/**
 * Feature-Driven Campaign Templates
 * Promote platform features to drive user engagement and gig creation
 */

import { getEmailTemplate } from './shared.templates';

/**
 * Campaign for videographers: Promote AI playground for music video ideation
 */
export function getMusicVideoPlaygroundCampaignTemplate(
  ...
  const content = `
    ...
    <div style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin: 30px 0; text-align: center;">
      ...
      <p style="color: #4b5563; font-size: 15px; margin: 0; font-style: italic;">
        "The AI playground helped me visualize my entire music video concept in under 10 minutes. I found the perfect dancers within 24 hours!"
      </p>
      <p style="color: #9ca3af; font-size: 13px; margin: 10px 0 0 0;">— Marcus D., Music Video Director</p>
    </div>
  `;
...
/**
 * Campaign for photographers: Promote AI playground for photoshoot ideation
 */
export function getPhotoshootPlaygroundCampaignTemplate(
  recipientName: string,
  userEmail?: string,
  userId?: string
): string {
...
/**
 * Campaign for photographers: Promote playground for photoshoot concepts
 */
export function getPhotographyPlaygroundCampaignTemplate(
  recipientName: string,
  userEmail?: string,
  userId?: string
): string {
  const content = `
    ...
    <div style="background-color: #f9fafb; border-radius: 8px; padding: 20px; margin: 30px 0;">
      <p style="color: #1a1a1a; font-size: 15px; font-weight: 600; margin: 0 0 12px 0; text-align: center;">
        Example Use Cases:
      </p>
      <ul style="margin: 0; padding-left: 20px; color: #4b5563; font-size: 14px;">
        <li style="margin-bottom: 8px;"><strong>Editorial Fashion:</strong> Generate high-fashion concepts, book editorial models</li>
        <li style="margin-bottom: 8px;"><strong>Beauty Campaigns:</strong> Create beauty shot references, find perfect beauty models</li>
        <li style="margin-bottom: 8px;"><strong>Lifestyle Shoots:</strong> Visualize authentic moments, match with lifestyle talent</li>
        <li style="margin-bottom: 8px;"><strong>Commercial Work:</strong> Present concepts to clients, find professional models</li>
      </ul>
    </div>
  `;
...
/**
 * Campaign for all creatives: Promote playground features
 */
export function getMoodboardFeatureCampaignTemplate(
  recipientName: string,
  recipientRole: 'photographer' | 'videographer' | 'creative',
  userEmail?: string,
  userId?: string
): string {
  const roleContent = {
    photographer: {
      ...
      title: 'Photographers',
      benefit: 'photoshoots',
      example: 'editorial fashion, portrait sessions, commercial campaigns'
    },
    videographer: {
      icon: '🎬',
      title: 'Videographers',
      benefit: 'video productions',
      example: 'music videos, commercials, short films'
    },
    creative: {
      icon: '🎨',
      title: 'Creatives',
      benefit: 'projects',
      example: 'any creative collaboration'
    }
  };

  const role = roleContent[recipientRole];

  const content = `
    <div style="text-align: center; margin-bottom: 30px;">
      <div style="font-size: 80px; margin-bottom: 15px;">${role.icon}</div>
      <h1 style="color: #1a1a1a; font-size: 32px; font-weight: 700; margin: 0 0 10px 0;">Moodboards That Get You Booked</h1>
      <p style="color: #6b7280; font-size: 18px; margin: 0;">For ${role.title} on Preset</p>
    </div>
    ...
    <div style="background-color: #f9fafb; border-left: 4px solid #00876f; padding: 20px; margin: 30px 0; border-radius: 4px;">
      <p style="color: #1a1a1a; font-size: 15px; font-weight: 600; margin: 0 0 10px 0;">
        💡 Pro Tip: Use AI Playground for ${role.benefit}
      </p>
      <p style="color: #4b5563; font-size: 14px; margin: 0; line-height: 1.6;">
        Try prompts like: <em>"${role.example}"</em> - the AI generates multiple visual concepts you can refine and use in your moodboard!
      </p>
    </div>
    ...
    <div style="text-align: center; margin: 40px 0;">
      <div style="display: inline-block; text-align: left;">
        ...
        <a href="https://presetie.com/gigs/create" 
           style="display: block;
                  background: #ffffff; 
                  color: #00876f; 
...
/**
 * Campaign focusing on gig creation with AI features
 */
export function getCreateGigWithAICampaignTemplate(
  recipientName: string,
  recipientRole: 'CONTRIBUTOR',
  shootType: string,  // e.g., "music video", "fashion shoot", "portrait session"
  userEmail?: string,
  userId?: string
): string {
  const content = `
    <div style="text-align: center; margin-bottom: 30px;">
      <h1 style="color: #1a1a1a; font-size: 32px; font-weight: 700; margin: 0 0 10px 0;">Create Your Next ${shootType.charAt(0).toUpperCase() + shootType.slice(1)}</h1>
    ...
    <div style="margin: 30px 0;">
      <div style="background-color: #ffffff; border: 2px solid #00876f; border-radius: 12px; padding: 25px; margin-bottom: 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        ...
        <p style="color: #4b5563; font-size: 15px; margin: 0; text-align: center; line-height: 1.6;">
          Open the <strong>AI Playground</strong>. Describe your ${shootType} concept. The AI generates multiple visual references instantly.
        </p>
      </div>
    ...
    <div style="text-align: center; margin: 40px 0;">
      <a href="https://presetie.com/playground" 
         style="display: inline-block; 
                ...
                font-size: 20px;
                box-shadow: 0 6px 20px rgba(0, 135, 111, 0.5);">
        Start Your ${shootType.charAt(0).toUpperCase() + shootType.slice(1)}
      </a>
    </div>
