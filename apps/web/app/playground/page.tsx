...
export default function PlaygroundPage() {
  ...
  const performAdvancedEdit = async () => {
    ...
    try {
      ...
      alert(`Image ${editType} completed! Used ${creditsUsed} credits.`)
    } catch (error) {
      console.error('Advanced edit failed:', error)
      alert('Advanced edit failed: ' + (error instanceof Error ? error.message : 'Unknown error'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const generateSequentialImages = async () => {
    ...
    try {
      ...
      alert(`Generated ${images.length} sequential images! Used ${creditsUsed} credits.`)
    } catch (error) {
      console.error('Sequential generation failed:', error)
      alert('Sequential generation failed: ' + (error instanceof Error ? error.message : 'Unknown error'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const generateVideo = async () => {
    ...
    try {
      ...
      alert(`Video generated successfully! Used ${creditsUsed} credits.`)
    } catch (error) {
  ...
  const generateSequentialEdits = async () => {
    ...
    try {
      ...
      alert(`Generated ${images.length} sequential edited images! Used ${creditsUsed} credits.`)
    } catch (error) {
      console.error('Sequential editing failed:', error)
      alert('Sequential editing failed: ' + (error instanceof Error ? error.message : 'Unknown error'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const generateStyleVariations = async () => {
    if (!selectedImage) {
      alert('Please select an image to generate style variations')
      return
    }
    ...
    if (stylesToUse.length === 0) {
      alert('Please select at least one style or style preset')
      return
    }
    ...
    try {
      ...
      if (errors && errors.length > 0) {
        alert(`Style variations completed with ${errors.length} errors. Generated ${results.length} variations. Used ${creditsUsed} credits.`)
      } else {
        alert(`Generated ${results.length} style variations! Used ${creditsUsed} credits.`)
      }
    } catch (error) {
      console.error('Style variation generation failed:', error)
      alert('Style variation generation failed: ' + (error instanceof Error ? error.message : 'Unknown error'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const performBatchEdit = async () => {
    if (batchImages.length === 0) {
      alert('Please add images for batch editing')
      return
    }
    ...
    try {
      ...
      if (errors && errors.length > 0) {
        alert(`Batch editing completed with ${errors.length} errors. Processed ${results.length} images. Used ${creditsUsed} credits.`)
      } else {
        alert(`Batch editing completed! Processed ${results.length} images. Used ${creditsUsed} credits.`)
      }
    } catch (error) {
      console.error('Batch editing failed:', error)
      alert('Batch editing failed: ' + (error instanceof Error ? error.message : 'Unknown error'))
    } finally {
      setLoading(false)
    }
  }
