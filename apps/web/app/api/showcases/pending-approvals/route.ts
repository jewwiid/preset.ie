...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedShowcases = showcases?.map(showcase => ({
      ...
      gig_title: showcase.gigs?.[0]?.title || 'Unknown Gig',
      creator_user_id: showcase.creator?.[0]?.id || '',
      creator_name: showcase.creator?.[0]?.display_name || 'Unknown',
      creator_handle: showcase.creator?.[0]?.handle || 'unknown',
      creator_avatar_url: showcase.creator?.[0]?.avatar_url || '',
