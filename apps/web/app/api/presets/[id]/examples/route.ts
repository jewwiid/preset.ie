...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      if (presetDataError || !presetData) {
        ...
        if (cinematicError || !cinematicPresetData) {
          ...
          // Fetch examples from media table where style matches cinematic preset name
    ...
    // Format examples from media table
    let formattedExamples = examples;
    
    if (examples.length > 0 && examples[0].path) {
      // These are from media table, need to format them
      formattedExamples = await Promise.all(
        examples?.map(async (example) => {
          ...
          return {
            ...
            is_verified: false, // Media table examples are not verified by default
            creator: {
