            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-1 text-sm font-medium transition-colors p-1.5 rounded-md ${
                  isMuted
                    ? 'text-muted-foreground-500 hover:text-muted-foreground-600 bg-muted-100'
          ...
          <div className="max-h-80 overflow-y-auto">
            {loading ? (
              ...
              <div className="text-center p-8">
                {isMuted ? (
                  <BellOff className="w-12 h-12 text-muted-foreground-300 mx-auto mb-4" />
                ) : (
                  <Bell className="w-12 h-12 text-muted-foreground-300 mx-auto mb-4" />
                )}
              ...
              <div className="divide-y divide-muted-primary/20">
                {notifications.slice(0, 10).map((notification) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-shrink-0">
                        {notification.avatar_url ? (
                          ...
                          <div className="w-8 h-8 bg-muted-100 rounded-full flex items-center justify-center text-sm">
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex-1">
                            ...
                            {notification.message && (
                              <p className="text-sm text-muted-foreground-600 mt-1 leading-5">
                                {notification.message}
                              </p>
                            )}
                          ...
                          <div className="flex items-center gap-1">
                            {!notification.read_at && (
                              <button
                                ...
                                className="p-1 text-muted-foreground-400 hover:text-primary transition-colors"
                                title="Mark as read"
                              >
                            ...
                            <button
                              ...
                              className="p-1 text-muted-foreground-400 hover:text-destructive-600 transition-colors"
                              title="Dismiss"
                            >
                              <X className="w-4 h-4" />
                            </button>
                    ...
                    {/* Unread indicator */}
                    {!notification.read_at && (
                      <div className="absolute left-2 top-6 w-2 h-2 bg-primary-500 rounded-full"></div>
                    )}
                  </div>
                ))}
              </div>
