-- Add missing presets with correct categories (safe version)
-- This script checks for existing presets before adding to avoid duplicates

-- First, ensure the category constraint is updated (run fix_category_constraint.sql first)

-- Add missing instant film presets (only if they don't exist)
INSERT INTO presets (
...
) AS new_presets(name, description, category, prompt_template, negative_prompt, style_settings, technical_settings, ai_metadata, is_public, is_featured)
WHERE NOT EXISTS (
  SELECT 1 FROM presets 
  WHERE presets.name = new_presets.name
);
