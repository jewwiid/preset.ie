...
ALTER TABLE users_profile 
ADD CONSTRAINT IF NOT EXISTS check_years_experience_positive 
    CHECK (years_experience >= 0),
ADD CONSTRAINT IF NOT EXISTS check_hourly_rate_valid 
    CHECK (hourly_rate_min IS NULL OR hourly_rate_max IS NULL OR hourly_rate_min <= hourly_rate_max),
ADD CONSTRAINT IF NOT EXISTS check_travel_radius_positive 
    CHECK (travel_radius_km >= 0),
ADD CONSTRAINT IF NOT EXISTS check_turnaround_days_positive 
    CHECK (typical_turnaround_days IS NULL OR typical_turnaround_days > 0),
ADD CONSTRAINT IF NOT EXISTS check_height_positive 
    CHECK (height_cm IS NULL OR height_cm > 0),
ADD CONSTRAINT IF NOT EXISTS check_account_status_valid 
    CHECK (account_status IN ('pending_verification', 'active', 'suspended', 'banned'));
...
-- (The existing policies should already cover these fields since they're part of the same table)
